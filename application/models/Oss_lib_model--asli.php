...
class Oss_lib_model extends CI_Model {
	...
	function get_data_pemilik($id_pemilik=null)
	{
		$sql = "SELECT a.*,b.tgl_imb,b.dir_file_imb
				FROM tmdatapemilik a
				LEFT JOIN tmdatabangunan b ON (b.id=a.id)
				WHERE (1=1) ";

		if (trim($id_pemilik) != '' && $id_pemilik != null ) $sql .= " AND a.id = '$id_pemilik' ";

		return $this->db->query($sql);

	}
	
	function insert_data_log($dataIn)
	{
		$this->db->insert('log_sso_oss',$dataIn);
