<div class="portlet box blue">
	<div class="portlet-title">
		<div class="caption"><i class="fa fa-globe"></i>List Data Verifikasi Konsultasi Bangunan Bertahap</div>
		<div class="tools"><a href="javascript:;" class="reload"></a></div>
	</div>
	<div class="portlet-body">
		<div class="form-actions">
			<?php echo form_open('DinasTeknis/VerifikasiBertahap', array('name' => 'frmListVerifikasiBertahap', 'id' => 'frmListVerifikasiBertahap')); ?>
			<div class="row">
				<div class="col-md-12">
					...
					foreach ($Verifikasi->result() as $Konsultasi) { ?>
						...
						<tr class="<?= $clss ?>">
							...
							<?php if ($Konsultasi->status == 1) { ?>
								<td align="center">
									<a href="#" onClick="href='<?php echo site_url('DinasTeknis/FormVerifikasiBertahap/' . $Konsultasi->id); ?>'" class="btn btn-success btn-sm" title="Verifikasi Data" id="tombolver" data-toggle="modal" data-target="#modal-edit"><span class="glyphicon glyphicon-edit"></span></a>
								</td>
...
<!-- /.modaledit -->
<div id="modal-edit" class="modal fade" tabindex="-1" data-width="90%" aria-hidden="true" role="dialog" data-backdrop="static" data-keyboard="false" data-focus-on="input:first">
								...
								<div class="tab-content">
									...
									<div class="tab-pane fade" id="tab_2_4">
										<table class="table table-bordered table-striped table-hover">
											...
											<tbody class="data-history"></tbody>
										</table>
									</div>
								</div>
							</div>
		...
		$(document).on('click', '.detail-verifikasi', function(e) {
			...
			$.ajax({
              ...
              $('.prasarana').css('display', 'none');
						...
						var tableHistory;
						let numTableHistory = 1;
						response.history.forEach(obj => {
							tableHistory += '<tr>';
							tableHistory += `<td style="text-align:center;">${numTableHistory++}</td>`;
							tableHistory += `<td>${obj.tgl_status}</td>`;
							tableHistory += `<td>${obj.catatan}</td>`;
							let berkas = obj.dir_file == false ? 'Tidak ada dokumen' : `<a href="javascript:void(0);" class="btn default btn-xs blue-stripe" title="Lihat Berkas" onclick="javascript:popWin('${site_url}${obj.dir_file}')"><span class="glyphicon glyphicon-file"></span>Lihat</a>`;
							tableHistory += `<td style="text-align:center;">${berkas}</td>`;
							tableHistory += `<td>${obj.post_by}</td></tr>`;
						});
						$('.data-history').html(tableHistory);
						var tableArsitektur;
						let numArsitektur = 1;
