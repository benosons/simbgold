...
<div class="portlet box blue">
    <div class="portlet-title">
        <div class="caption"><i class="fa fa-globe"></i>List Validasi Inspeksi Kepala Dinas Teknis Bangunan Gedung Baru</div>
                        ...
                        <tr class="<?= $clss ?>">
                            ...
                            <?php } else {?>
                                <td>
                                    ...
                                    <a href="<?php echo site_url('Pemeriksaan/Catatan/' . $Kadis->id); ?>" class="btn btn-info btn-sm" title="Lihat Data" id="tombolver" data-toggle="modal" data-target="#modal-edit2"><span class="fa fa-file"></span> Lihat Catatan</a>
                                </td>
                            <?php }?>
                            <?php if ($Kadis->status == 19) { ?>
                                <td align="center">
                                    ...
                                    <div class="data-action"></div><br>
                                    <a href="<?php echo site_url('Kadintek/RollbackKadis/' . $Kadis->id); ?>" class="btn btn-warning btn-sm" onclick="return confirm('Yakin Mengembalikan Permohonan ini ke Proses Perhitungan Retribusi?')"title="Dikembalikan ke Proses Perhitungan Retribusi"><span class="glyphicon glyphicon-edit"></span>
                                </td>
                            <?php } else { ?>
                                <td align="center">
...
<div id="dialog-popup" class="modal fade bs-modal-sm" data-width="55%" tabindex="-1" aria-hidden="true" role="dialog" data-backdrop="static" data-keyboard="false">
    <div class="modal-content">
        <form action="<?php echo site_url('Kadintek/FormVerifikasiBangunanGedung'); ?>" class="form-horizontal" role="form" method="post">
...
<script>
    function GetCetakRekomtek(id) {
        var url = "<?php echo base_url() . index_page() ?>Dokumen/CetakDokumenSertifikatLaikFungsiBangunanGedung/" + id;
        swin = window.open(url, 'win', 'scrollbars,width=1000,height=600,top=80,left=140,status=yes,toolbar=no,menubar=yes,location=no');
        swin.focus();
    }
    function GetVerifikasi(id) {
        $("#MyModalBody").html('<iframe src="<?php echo base_url(); ?>Dokumen/CetakDokumenSertifikatLaikFungsiBangunanGedung/' + id + '" frameborder="no" width="860" height="540"></iframe>');
        $('[name="id"]').val(id);
    }
</script>
<script>
