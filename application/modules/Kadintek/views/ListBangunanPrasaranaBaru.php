...
<div class="portlet box blue">
    <div class="portlet-title">
        <div class="caption"><i class="fa fa-globe"></i>List Validasi Inspeksi Kepala Dinas Teknis Bangunan Prasarana Baru</div>
                ...
                <?php if ($Kadis->num_rows() > 0) {
                    ...
                    foreach ($Kadis->result() as $Kadis) { ?>
                        ...
                        <tr class="<?= $clss ?>">
                            ...
                            <td align="center"><?php echo $Kadis->no_izin_pbg; ?></td>
                            ...
                            <?php if ($Kadis->status == 19) { ?>
                                <td align="center">
                                    ...
                                    <?php if($Kadis->imb == 1){ ?>
                                        <a href="<?php echo site_url('Kadintek/RollbackKadisEksis/' . $Kadis->id); ?>" class="btn btn-warning btn-sm" onclick="return confirm('Yakin Mengembalikan Permohonan ini ke Proses Perhitungan Retribusi?')"title="Dikembalikan ke Proses Perhitungan Retribusi"><span class="glyphicon glyphicon-edit"></span>
                                    <?php }else{ ?>
                                        <a href="<?php echo site_url('Kadintek/RollbackKadisEksis/' . $Kadis->id); ?>" class="btn btn-warning btn-sm" onclick="return confirm('Yakin Mengembalikan Permohonan ini ke Proses Perhitungan Retribusi?')"title="Dikembalikan ke Proses Perhitungan Retribusi"><span class="glyphicon glyphicon-edit"></span>
                                    <?php } ?>
                                </td>
                            <?php } else { ?>
                                <td align="center">
                                    <a href="#" onclick="GetCetakRekomtek(<?php echo $Kadis->id ?>)" class="btn btn-info btn" title="Cetak Rekomtek" id="tombolinver"><span class="glyphicon glyphicon-print"> Dokumen SLF</span></a>
                                </td>
                            <?php } ?>
                        </tr>
                <?php }
...
<div id="dialog-popup" class="modal fade bs-modal-sm" data-width="55%" tabindex="-1" aria-hidden="true" role="dialog" data-backdrop="static" data-keyboard="false">
    <div class="modal-content">
        <form action="<?php echo site_url('Kadintek/FormVerifikasiBangunanPrasarana'); ?>" class="form-horizontal" role="form" method="post">
...
<script>
    function GetCetakRekomtek(id) {
        var url = "<?php echo base_url() . index_page() ?>Dokumen/CetakDokumenSertifikatLaikFungsiBangunanPrasarana/" + id;
        swin = window.open(url, 'win', 'scrollbars,width=1000,height=600,top=80,left=140,status=yes,toolbar=no,menubar=yes,location=no');
        swin.focus();
    }
    function GetVerifikasi(id) {
        $("#MyModalBody").html('<iframe src="<?php echo base_url(); ?>Dokumen/CetakDokumenSertifikatLaikFungsiBangunanPrasarana/' + id + '" frameborder="no" width="860" height="540"></iframe>');
        $('[name="id"]').val(id);
    }
</script>
<script>
