				...
				<?php if ($ssrd->num_rows() > 0){	
					...
					foreach ($ssrd->result() as $ssrd) { ?>
						...
						<tr class="<?=$clss?>">
							...
							<?php  if ($ssrd->status == 12) { ?>
								<td align="center">
									<a href="#" onClick="href='<?php echo site_url('Validasipembayaran/FormValidasiRetribusi/'.$ssrd->id);?>'" class="btn btn-danger btn-sm" title="Validasi Kadis" id="tombolver" data-toggle="modal" data-target="#modal-edit"><span class="glyphicon glyphicon-edit"></span></a>
                                    <div class="data-action"></div><br>
                                    <a href="<?php echo site_url('Penerbitan/RollbackSSRD/' . $ssrd->id); ?>" class="btn btn-warning btn-sm" onclick="return confirm('Yakin Mengembalikan Permohonan ini ke Proses Pengiriman SKRD?')"title="Dikembalikan ke Proses Penagihan Retribusi"><span class="glyphicon glyphicon-edit"></span>
                                </td>
							<?php } else { ?>
								<td align="center">
									<a href="javascript:;" class="btn btn-info btn-sm detail-retribusi" title="Lihat Data" data-id="<?= $ssrd->id ?>"><span class="fa fa-file"></span> Lihat Disini</a>
