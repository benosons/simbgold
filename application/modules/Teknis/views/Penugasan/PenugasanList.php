<div class="portlet box blue">
	<div class="portlet-title">
		<div class="caption">
			<?php $kategori = @$_GET['id_fungsi_bg'];
			if ($kategori != NULL) {
				if ($kategori == 0) {
					$res = 'Konsultasi';
				} else if ($kategori == 1) {
			...
			<i class="fa fa-globe"></i>List Data Penugasan Pemeriksan Dokumen <?php echo $res ?>
		</div>
		<div class="tools">
...
<script>
	...
	$(document).ready(function() {
		...
		$(document).on('click', '.form-penugasan', function(e) {
			...
			$.ajax({
				type: "POST",
				url: `${site_url}Teknis/FormPenugasan`,
				dataType: 'json',
				data: {
					id: dataDetail
				},
		...
		$(document).on('click', '.simpan-penugasan', function(e) {
			...
			if (totalCeklist.length > 0) {
				$.ajax({
					type: 'POST',
					url: `${base_url}Teknis/SimpanPenugasan`,
					data: {
