...
$pdf->AddPage();
if($result_list['tgl_imb'] >"2021-08-04"){
...
//$pdf->Cell(0,10,'IZIN MENDIRIKAN BANGUNAN GEDUNG',0,1,'C');
if($result_list['tgl_imb'] >"2021-08-03"){
...
if ($count_tanah>1) {
  ...
  //$pdf->Cell(17,5,(isset($row['luas_tanah']) ? number_format($row['luas_tanah']'','.') : '[ LUAS TANAH KOSONG ]'),0,0);
  $pdf->Cell(17,5,number_format($row['luas_tanah'],2,',','.'),0,0);
  $pdf->Cell(4,5,' m',0,0);
  $pdf->subWrite(5,'2','',6,4);
  $pdf->Cell(10,5,'',0,1);
  }
...
if ($result_list['id_fungsi_bg'] != 1) {
	$pdf->MultiCell(0,5,'Berita Acara Hasil Pemeriksaan Dokumen Rencana Teknis Tim Ahli Bangunan Kota Palu Gedung Nomor '.$resultHasil['no_sk_tk'].' tanggal '.$tgl_teknis.' (untuk Bangunan Gedung Kepentingan Umum)',0,'J',0);
} else {
	$pdf->MultiCell(0,5,'Berita Acara Hasil Pemeriksaan Dokumen Rencana Teknis Tim Teknis Dinas Penataan Ruang dan Pertanahan Kota Palu Nomor '.$resultHasil['no_sk_tk'].' tanggal '.$tgl_teknis.' (untuk Bangunan Gedung Bukan Kepentingan Umum)',0,'J',0);
}

/*if($result_list['id_permohonan'] !='39415' )
{
	
}else if($result_list['id_permohonan'] =='55177'){
	$pdf->AddPage();
}else{
	$pdf->AddPage();
}*/
if($result_list['id_permohonan'] =='55177'){
	$pdf->AddPage();
}else{
