...
if($bg['permohonan_slf'] =='1'){
    ...
    $pdf->Cell(42, 5, "", 0, 0, "L");
    $pdf->Cell(50, 5, "Klasifikasi bangunan Gedung", 0, 0, "L");
    $pdf->Cell(0, 5, ": Tidak Sederhana", 0, 1, "L");
...
$pdf->Cell(20, 5, ": " .number_format($bg['luas_bgp'],2,',','.'), 0, 0, "L");
$pdf->Cell(5,5,'m',0,0);
$pdf->subWrite(1,'2','',6,4);
$pdf->Cell(10,5,'',0,1);
$pdf->Cell(41, 5, "", 0, 0, "L");
$j=0;
$k=1;
if ($count_tanah>1) {
    ...
    foreach ($result_tanah as $key => $row) {
        ...
        $pdf->Cell(2,5,':',0,0);
        $pdf->MultiCell(60,5,(isset($row['atas_nama_dok']) ? $row['atas_nama_dok'] : '[ PEMILIK TANAH KOSONG ]'),0,"L",0);
        if ($row['hat'] != 0) {
            $pdf->Cell(100,5,'',0,0);
...
$pdf->MultiCell(0, 5, "Besarnya retribusi yang harus dibayar oleh pemohon sebagaimana Dimaksud dalam Lampiran d Keputusan ini sebesar:", 0, "L", 0);
$pdf->Cell(55, 5, "", 0, 0, "L");
...
if($bg['permohonan_slf'] =='1'){
    ...
    $pdf->Cell(0, 5, $klasifikasi, 0, 1, "C");
} else if ($bg['permohonan_slf'] =='2'){
    $pdf->Cell(0, 5, "Fungsi Bangunan Gedung", 0, 1, "C");
    $pdf->Cell(0, 5, "Fungsi Prsarana", 0, 1, "C");
    $pdf->Cell(0, 5, "Klasifikasi Bangunan Gedung", 0, 1, "C");
