...
<div class="portlet box blue">
	<div class="portlet-title">
		<div class="caption"><i class="fa fa-globe"></i>List Validasi Pembayaran Retribusi</div>
	...
	<div class="portlet-body">
		<table class="table table-striped table-bordered table-hover" id="sample_1">
            ...
            <tbody>
				<?php if ($ssrd->num_rows() > 0){	
					$no = 1;
					foreach ($ssrd->result() as $ssrd) { ?>
						<?php if($ssrd->status == '12'){
							$clss = "info";
						}elseif($ssrd->status == '14'){
							$clss = "warning";
						}else{
							$clss = "success";
						} ?>
						<tr class="<?=$clss?>">
							...
							<td><?php echo $ssrd->nm_konsultasi;?></td>
							<td align="center"><?php echo $ssrd->no_konsultasi;?></td>
							<td align="left"><?php echo $ssrd->nm_pemilik;?></td>
							<td><?php echo $ssrd->almt_bgn;?></td>
							<td>
								<a href="javascript:;" class="btn btn-info btn-sm detail-retribusi" title="Lihat Data" data-id="<?= $ssrd->id ?>"><span class="fa fa-file"></span> Lihat Disini</a>
							</td>
							<td align="center">
								<?php if($ssrd->status == 12){
									$class = "label label-sm label-warning";
									$syarat = "Validasi Pembayaran";
								}else {
							...
							<?php  if ($ssrd->status == 12) { ?>
								<td align="center">
									<a href="#" onClick="href='<?php echo site_url('VerifikasiRetribusi/FormVerifikasiRetribusi/'.$ssrd->id);?>'" class="btn btn-danger btn-sm" title="Validasi Kadis" id="tombolver" data-toggle="modal" data-target="#modal-edit"><span class="glyphicon glyphicon-edit"></span></a>
                                    <div class="data-action"></div><br>
                                    <a href="<?php echo site_url('VerifikasiRetribusi/RollbackSSRD/' . $ssrd->id); ?>" class="btn btn-warning btn-sm" onclick="return confirm('Yakin Mengembalikan Permohonan ini ke Proses Pengiriman SKRD?')"title="Dikembalikan ke Proses Penagihan Retribusi"><span class="glyphicon glyphicon-edit"></span>
                                </td>
							<?php } else { ?>
								<td align="center">
									<a href="javascript:;" class="btn btn-info btn-sm detail-retribusi" title="Lihat Data" data-id="<?= $ssrd->id ?>"><span class="fa fa-file"></span> Lihat Disini</a>
									<!--<a href="#" onClick="href='<?php echo site_url('DataDetail/DataKonsultasi/'.$ssrd->id);?>'" class="btn btn-danger btn-sm" title="Lihat Data" id="tombolinver" data-toggle="modal" data-target="#modal-edit"><span class="glyphicon glyphicon-user"></span></a>-->
								</td>
							<?php } ?>
						</tr>
					<?php }
...
{
	$("#MyModalBody").html('<iframe src="<?php echo base_url();?>Penerbitan/CetakPersetujuanBangunanGedung/'+id+'" frameborder="no" width="860" height="540"></iframe>');
	$('[name="id"]').val(id) ; 
}
</script>
<script>
