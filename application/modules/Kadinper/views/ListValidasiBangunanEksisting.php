...
<div class="portlet box blue">
	<div class="portlet-title">
		<div class="caption"><i class="fa fa-globe"></i>List Validasi Bangunan Gedung Eksisting Oleh Kepala Dinas Perizinan</div>
            ...
            <tbody>
				<?php if($Kadis->num_rows() > 0){
                    ...
                    <tr class="<?=$clss?>">
						...
						<td><?php echo $Kadis->no_izin_pbg;?></td>
                        ...
                        <?php if ($Kadis->status == 13) { ?>
							<td align="center">
                                ...
                                <a href="<?php echo site_url('Kadinper/RollbackKadis/' . $Kadis->id); ?>" class="btn btn-warning btn-sm" onclick="return confirm('Yakin Mengembalikan Permohonan ini ke Proses Validasi Pembayaran Retribusi?')"title="Dikembalikan ke Proses Validasi Pembayaran Retribusi"><span class="glyphicon glyphicon-edit"></span>
                            </td>
                        <?php } else { ?>
							<td align="center">
                                <a  href="#" onclick="GetCetakSertifikatLaikFungsi(<?php echo $Kadis->id ?>)" class="btn btn-info btn" title="Cetak IMB" id="tombolinver" ><span class="glyphicon glyphicon-print"> Terverifikasi (Lihat PBG)</span></a>
							</td>
                        <?php } ?>
					</tr>
                    <?php }
...
<div class="modal fade" id="dialog-popup" data-width="53%" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<form action="<?php echo site_url('Kadinper/ValidasiFormEksis'); ?>" class="form-horizontal" role="form" method="post">
...
{
	var url = "<?php echo base_url() . index_page() ?>Dokumen/CetakSertifikatLaikFungsi/"+id;
	swin = window.open(url,'win','scrollbars,width=1000,height=600,top=80,left=140,status=yes,toolbar=no,menubar=yes,location=no');
	swin.focus();
}
function GetCetakSertifikatLaikFungsi(id)
{
	$("#MyModalBody").html('<iframe src="<?php echo base_url();?>Dokumen/CetakSertifikatLaikFungsi/'+id+'" frameborder="no" width="960" height="540"></iframe>');
	$('[name="id"]').val(id) ; 
}
</script>
<script>
