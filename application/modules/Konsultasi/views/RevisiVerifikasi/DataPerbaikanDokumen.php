						...
						<?php if (!empty($DataTkTanah)) {
							...
							foreach ($DataTkTanah->result() as $key) {
								...
								<tr class="<?= $clss ?>">
									...
									<td align="center">
										<?php echo form_open_multipart('Konsultasi/UploadPerbaikan/'.$id .'/'.$key->id_detail .'/1/'.$id_teknis, array('name' => 'frmup' . $no, 'id' => 'frmup' . $no)); ?>
										<?php if ($status_ver !='1'){ ?>
					...
					<?php if (!empty($DataMPE)) {
						...
						foreach ($DataMPE->result() as $key) {
							...
							<tr class="<?= $clss ?>">
								...
								<td align="left"><?php echo $key->keterangan; ?></td>
								<td align="center">
									<?php echo form_open_multipart('Konsultasi/UploadPerbaikan/'.$id .'/'.$key->id_detail .'/4/'.$id_teknis, array('name' => 'frmup' . $no, 'id' => 'frmup' . $no)); ?>
									<?php if ($status_ver !='1'){ ?>
											<?php if($dir_file == null)  {?>
												<input type="file" name="d_file" id="d_file" placeholder="Unggah Berkas Disini" accept="application/pdf" onchange="form.submit()">
											<?php } else { ?>
