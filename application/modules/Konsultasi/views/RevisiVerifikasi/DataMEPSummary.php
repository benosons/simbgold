					...
					<?php if (!empty($DataMPE)) {
						...
						foreach ($DataMPE->result() as $key) {
							...
							<tr class="<?= $clss ?>">
								...
								<td align="center">
									<?php echo form_open_multipart('Konsultasi/UploadPerbaikanMEP/'.$id.'/'.$key->id_detail.'/4/'.$id_teknis,array('name'=>'frmup'.$no, 'id'=>'frmup'.$no)); ?>		
									<?php if ($status_ver !='1'){ ?>
										...
										<?php } else { ?>
											...
											|
											<a href="<?php echo site_url('Konsultasi/DeleteTeknisRevMEP/' . $id_teknis . '/tek/' . $id); ?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin Hapus Data ini?')" title="Hapus Data"><span class="glyphicon glyphicon-trash"></span></a>
										<?php } ?>
