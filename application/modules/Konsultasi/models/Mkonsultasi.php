...
class Mkonsultasi extends CI_Model
{
	...
	public function getDataKonsultasi($select = "a.*,c.*", $user_id = '', $id_permohonan = '')
	{
		$this->db->select($select, FALSE);
		if ($user_id != null || trim($user_id) != '')  $this->db->where('a.user_id', $user_id);
	...
	public function getBangunan($select = "*", $id)
	{
		...
		$this->db->join('tr_konsultasi i', 'a.id_jenis_permohonan = i.id', 'LEFT');
	...
	public function getCatatanKonsultasi($select = "a.*", $id = '')
	{
		$this->db->select($select, FALSE);
		if ($id != null || trim($id) != '')  $this->db->where('a.id', $id);
		$query 	= $this->db->get('tm_perbaikan_sidang a');
		return $query;
	}
	...
	{
		$this->db->select('a.*,b.*,c.nm_konsultasi,d.*,e.*,f.*,k.nama_kelurahan,g.nama_kecamatan as nm_kec_bgn,
		h.nama_kabkota as nm_kabkot_bgn,i.nama_provinsi as nm_prov_bgn, j.nama_kelurahan as nm_kel_bgn,l.dir_file');
		$this->db->from('tmdatapemilik a');
		$this->db->where('a.id', $id);
		$this->db->join('tmdatabangunan b', 'a.id = b.id', 'LEFT');
		$this->db->join('tr_konsultasi c', 'b.id_jenis_permohonan = c.id', 'LEFT');
	...
	function getRowKonsultasi($id)
	{
        ...
        c.nm_konsultasi,
        f.fungsi_bg,
        g.nama_kecamatan as nama_kec_bg,j.nama_kabkota as nama_kabkota_bg,k.nama_provinsi as nama_provinsi_bg,a.id_kelurahan,a.jns_pemilik,b.id_kel_bgn,b.id_jns_bg,b.id_klasifikasi,m.klasifikasi_bg,l.*,n.nama_kelurahan as nm_kelurahan');
		$this->db->from('tmdatapemilik a');
		$this->db->where('a.id', $id);
		$this->db->join('tmdatabangunan b', 'a.id = b.id', 'LEFT');
		$this->db->join('tr_konsultasi c', 'b.id_jenis_permohonan = c.id', 'LEFT');
	...
	public function getRowSKRD($id)
	{
		return $this->db->where('id_pbgnya', $id)->get('tmbayar');
	}
