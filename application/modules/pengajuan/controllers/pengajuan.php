...
class Pengajuan extends CI_Controller {
	...
	public function DataPermohonanIMB($user_id=null)
	{
		...
		$data['content'] = $this->load->view('pengajuan_imb/DataPengajuanIMB',$data,TRUE);
		$data['title']		=	'Daftar Permohonan IMB';
		$data['heading']	=	'';
	...
	public function FormPendaftaranIMB()
	{
		...
		if (isset($data['profile_user']->id_kabkota)) {
			...
			$data['daftar_idp']			= $this->mglobal->listidp('a.idp,a.tipe_protype', '', $kabkot);
		}
	...
	public function PermohonanImbForm()
	{
		...
		$pastiinada = $this->mglobals->getData('*', 'tm_imb_permohonan', array('id_permohonan' => $id_permohonan));
		if ($pastiinada->num_rows() > 0) {
		...
		if (isset($data['DataPermohonan']->id_kabkot_bg)) {
			...
			$idpnull = "pupr";
			$idpnya  = $data['DataPermohonan']->idp;
			$data['daftar_idp']				= $this->mglobal->listidp('a.idp,a.tipe_protipe', '', $kabkot);
  ...
  //create directory if not exist
		...
		if ((!$this->upload->do_upload($file_tanah)) && ($lam_tanah != '')) {
				...
				$this->session->set_flashdata('message','Jenis Berkas atau Ukuran Berkas tidak Sesuai !');
				$this->session->set_flashdata('status','warning');
				redirect('pengajuan/FormSummary/'.$id_permohonan.'#tab2');
		}elseif ((!$this->upload->do_upload($file_phat)) && ($lam_phat != '')) {
				$data['err_msg'] = $this->upload->display_errors('','');
				$this->session->set_flashdata('message','Jenis Berkas atau Ukuran Berkas tidak Sesuai !');
				$this->session->set_flashdata('status','warning');
	...
	public function removeDataPengajuan($id_permohonan)
	{
		//$id_permohonan	= $this->uri->segment(4);
		$process = $this->mpengajuan->removeDataPermohonan($id_permohonan);
		/*$thisdir = getcwd();
		$dirPath = $thisdir."/file/imb/pengajuan_imb/".$id_permohonan;
		rmdir($dirPath, 0777, true);  //Delete directory if not exist*/
		if (!$process) {
	...
	public function FormImbTeknis()
	{
		...
		if($id_permohonan != ''){
			...
			$filterProtype			= 'a.idp,a.dir_file';
			$data['DataProtype']	= $this->mpengajuan->getDataProtype($filterProtype,$id_permohonan);
		}
		$data['mpengajuan']			=	$this->mpengajuan;
	...
	{	
		...
		$data['id_syarat']			= $id_syarat;
				...
				$thisdir = getcwd();
				$dirPath = $thisdir."/file/imb/pengajuan_imb/".$id_permohonan."/data_teknis";
				if (!file_exists($dirPath)){
				//mkdir($dirPath, 0755, true);
  //create directory if not exist
				}
  ...
  //create directory if not exist
	...
	{	
		...
		$data['id_syarat']			= $id_syarat;
				...
				$thisdir = getcwd();
				$dirPath = $thisdir."/file/imb/pengajuan_imb/".$id_permohonan."/data_administrasi";
				if (!file_exists($dirPath)){
					//mkdir($dirPath, 0755, true);
  //create directory if not exist
				}
  ...
  //create directory if not exist
	...
	public function saveDataIMB()
	{
		...
		$no_kolektif	= $this->input->post('no_kolektif');
		...
		if ($id_jenis_bg == '4'){
			...
			if ($id_fungsi_bg =='1') {
				...
				$id_pemanfaatan_bg = '2';
				$jns_bangunan		= $this->input->post('jns_bangunan');
				$idp				= $this->input->post('nama_idp');
				$id_dok_tek			= $this->input->post('id_dok_tek');
				
			} elseif ($id_fungsi_bg =='6') {
			
				$id_pemanfaatan_bg = '1';
				$id_dok_tek		= '1';
				$jns_campur		= $this->input->post('dcampur');
				$jns_bangunan	= implode(", ", $jns_campur);
	...
	public function FormSummary()
	{
		...
		if($id_permohonan != ''){
			...
			$data['DataTanah']			= $this->mpengajuan->getDataTanah('a.*',$id_permohonan);
			$data['catatan']			= $this->mpengajuan->getDataCatatan('a.*',$id_permohonan);
  ...
  //create directory if not exist
	...
	function list_nib_popup($nib=null)
	{
		...
		$query = $this->mpengajuan->get_nibnya($nib);
	...
	public function getIDP()
	{
		...
		$pupr		= "pupr";
		$value		= array();
		$query		= $this->mglobal->listidp('a.idp,a.type_protype', '', $id_kabkot);
		$query2		= $this->mglobal->listidp('a.idp,a.type_protype', '', $pupr);
		
			foreach ($query->result() as $row) {
				$value[]	= array('idp' => $row->idp, 'type_protype' => $row->type_protype);
			}
