				...
				<div class="portlet box blue">
					...
					<div class="portlet box blue">
						<div class="portlet-body form">
							...
							<input type="hidden" class="form-control" value="<?php echo set_value('id_jenis_permohonan', (isset($id_jenis_permohonan) ? $id_jenis_permohonan : ''))?>" name="id_nama_permohonan" placeholder="id_nama_permohonan" autocomplete="off">
							</form>
											...
											if(!empty($MasterTeknis)){
													...
													foreach ($MasterTeknis->result() as $key) {
														if ($no % 2 == 0 )
															...
															if(!empty($DataTeknis)){
																if(!empty($DataTeknis)){
																		...
																		foreach ($DataProtype->result() as $keyProtype) {
																			$fileProtype = $keyProtype->dir_file;
																		}
																	}	
																}
																?>
														<tr class="<?=$clss?>" >
															...
															<td align="center">
																<?php echo form_open_multipart('pengajuan/persyaratan_submit/'.$id_permohonan.'/'.$key->id_persyaratan_detail.'/2/'.$id_administrasi,array('name'=>'frmup'.$no, 'id'=>'frmup'.$no)); ?>
																<? if($key->id_persyaratan_detail =='1858'){?>
																	<center><a class="btn default btn-xs blue-stripe" onClick="javascript:getProtype('<?php echo $fileProtype;?>')" src="<?php echo base_url()?>assets/images/pdf.gif" title='File Pdf' class='link' > 
																				Lihat
																	</a></center>
																<?}else{?>	
																	...
																	<center>
																		...
																		| 
																		<a href="<?php echo site_url('pengajuan/removeDataPersyaratan2/'.$id_teknis.'/2/'.$id_permohonan);?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin Hapus Data ini?')" title="Hapus Data"><span class="glyphicon glyphicon-trash"></span></a>
																	</center>	
																	<? }?>
																<? }?>
																<?php echo form_close(); ?>
					...
					<span class="input-group-btn">
						<button class="btn green" onClick="window.location.href = '<?php echo base_url();?>Pengajuan/FormImbAdministrasi/<?php echo $id_permohonan;?>';return false;">Kembali</button>
					</span>
					
					<span class="input-group-btn">
						<button class="btn green" onClick="window.location.href = '<?php echo base_url();?>pengajuan/FormPernyataan/<?php echo $id_permohonan;?>';return false;">Selanjutnya</button>
					</span>
					<center>
					<br>
				</div>
