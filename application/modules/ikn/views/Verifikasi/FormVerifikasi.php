						...
						<tbody>
							...
							if ($DataTanah->num_rows() > 0) {
								...
								foreach ($DataTanah->result() as $key) {
									...
									$filename = FCPATH . '/object-storage/file/Konsultasi/' . $key->id . '/data_tanah/' . $key->dir_file;
									$filenamephat = FCPATH . '/object-storage/file/Konsultasi/' . $key->id . '/data_tanah/' . $key->dir_file;
									$dirt = '';
						...
						<tbody>
							...
							if ($History->num_rows() > 0) {
								...
								foreach ($History->result() as $his) { 
									$filename = FCPATH . "/object-storage/dekill/Consultation/$his->dir_file";
									$dir = '';
									if (file_exists($filename)) {
										$dir = base_url('object-storage/dekill/Consultation/' . $his->dir_file);
									} else {
										$dir = base_url('object-storage/file/Konsultasi/' . $his->id . '/' . $his->dir_file);
									} ?>
									<tr>
										<td align="center"><?= $no; ?></td>
										<!--<td align="center"><?= $his->status; ?></td>-->
...
<script type="text/javascript">
	...
	function check_status(key, id, ss) {
		...
		if (document.getElementById(key).checked) {
			$.ajax({
				url: '<?php echo base_url('DinasTeknis/check_status/' . $this->uri->segment(3)) ?>/' + id + '/' + ss + '/',
				type: 'POST',
				dataType: 'html',
				data: $('form.form-horizontal').serialize(),
				cache: false,
			...
			$.ajax({
				url: '<?php echo base_url('DinasTeknis/uncheck_status/' . $this->uri->segment(3)) ?>/' + id + '/' + ss + '/',
				type: 'POST',
				dataType: 'html',
				data: $('form.form-horizontal').serialize(),
				cache: false,
