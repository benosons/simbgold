...
$retri = $result_retri->harga_retribusi;
...
if (trim($result_list->date_sk_tk) != '') {
    ...
    $tgl_tek = tgl_eng_to_ind($result_list->tgl_konsultasi);
    $tgl_tek2 = explode('-', $tgl_tek);
    $tgl_teknis = $tgl_tek2[0] . ' ' . $montharray[$tgl_tek2[1] - 1] . ' ' . $tgl_tek2[2];
}

/*$tgl_setuju = tgl_eng_to_ind($result_list->tgl_pbg);
...
$pdf->SetFont('ARIAL', '', 12);
$pdf->Cell(0, 5, 'NOMOR ' . (isset($result_list->no_konsultasi) ? $result_list->no_konsultasi : '[ NOMOR PBG KOSONG ]'), 0, 1, 'C');
$pdf->SetFont('ARIAL', '', 12);
...
$pdf->Cell(2, 5, ':', 0, 0);
$pdf->Cell(49, 5, $result_list->no_konsultasi, 0, 1, 'L');
...
$pdf->Cell(2, 5, ':', 0, 0);
if ($result_list->nm_pemilik != '') {
    $nama_pemilik = ucwords(strtoupper($result_list->nm_pemilik));
} else {
    $nama_pemilik = ucwords(strtolower($result_list->nm_pemilik));
}
...
$pdf->Cell(2, 5, ':', 0, 0);
if ($result_list->alamat != '') {
    $alamat_pemilik = $result_list->alamat;
} else {
    $alamat_pemilik = $result_list->alamat;
}
...
$pdf->Cell(2, 5, ':', 0, 0);
$pdf->MultiCell(0, 5, $result_list->nm_konsultasi, 0, 'J', 0);
...
$pdf->Cell(2, 5, ':', 0, 0);
$pdf->Cell(10, 5, (isset($result_list->jns_bangunan) ? $result_list->jns_bangunan : '[ JENIS BANGUNAN KOSONG ]'), 0, 1);

$pdf->Cell(33, 5, '', 0, 0);
$pdf->Cell(42, 5, 'Nama bangunan gedung', 0, 0, 'L');
$pdf->Cell(2, 5, ':', 0, 0);
$pdf->Cell(10, 5, (isset($result_list->nm_bgn) ? $result_list->nm_bgn : '[ NAMA BANGUNAN KOSONG ]'), 0, 1);

if ($result_list->tinggi_bgn == 0 || $result_list->tinggi_bgn == null) {
    $pdf->Cell(33, 5, '', 0, 0);
    $pdf->Cell(42, 5, 'Luas bangunan gedung', 0, 0, 'L');
    $pdf->Cell(2, 5, ':', 0, 0);
    $pdf->Cell(14, 5, (isset($result_list->luas_bgn) ? $result_list->luas_bgn : '[ LUAS BANGUNAN KOSONG ]'), 0, 0);
    if ($result_list->luas_bgn == 0 || $result_list->luas_bgn == null) {
        $pdf->Cell(6, 5, ' m', 0, 0);
    ...
    $pdf->Cell(2, 5, ':', 0, 0);
    $pdf->Cell(10, 5, (isset($result_list->tinggi_bgn) ? $result_list->tinggi_bgn : '[ TINGGI BANGUNAN KOSONG ]'), 0, 0);
    if ($result_list->tinggi_bgn != 0 || $result_list->tinggi_bgn != null) {
        $pdf->Cell(4, 5, 'm', 0, 0);
        $pdf->Cell(10, 5, '', 0, 1);
    }
...
$pdf->Cell(2, 5, ':', 0, 0);
$pdf->MultiCell(95, 5, (isset($result_list->almt_bgn) ? $result_list->almt_bgn : '[ LOKASI TANAH KOSONG ]'), 0, 1);
...
$pdf->Cell(42, 5, 'Nama  Pemilik', 0, 0, 'L');
$pdf->Cell(2, 5, ':', 0, 0);
$pdf->Cell(10, 5, (isset($result_list->nm_pemilik) ? $result_list->nm_pemilik : '[ NAMA PEMOHON KOSONG ]'), 0, 1);

$pdf->Cell(38, 5, '', 0, 0);
$pdf->Cell(42, 5, 'Alamat Pemilik', 0, 0, 'L');
$pdf->Cell(2, 5, ':', 0, 0);
$pdf->MultiCell(0, 5, (isset($result_list->alamat) ? $result_list->alamat : '[ NAMA PEMILIK  KOSONG ]') . ", " . (isset($result_list->nama_kecamatan) ? 'Kec. ' . ucwords(strtolower($result_list->nama_kecamatan)) . ', ' . ucwords(strtolower($result_list->nama_kabkota)) : '[ NAMA KECAMATAN/KABKOT KOSONG ]') . ", " . (isset($result_list->nama_provinsi) ? 'Prov. ' . ucwords(strtolower($result_list->nama_provinsi)) : '[ NAMA PROVINSI KOSONG ]'), 0, 'J', 0);
...
$pdf->Cell(0, 5, '', 0, 1);
$pdf->Cell(33);
//$pdf->Cell(0, 0, $pdf->Image(BASE_FILE_PATH . 'Konsultasi/QR_Code/'.$result_pbg->no_pbg'.png', $pdf->GetX(), $pdf->GetY(), 30, 30), 0, 1);
...
$pdf->Output('I', $result_list->no_konsultasi . '.pdf');
