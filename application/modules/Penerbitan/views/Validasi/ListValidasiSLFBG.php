<div class="portlet box blue">
	<div class="portlet-title">
		<div class="caption"><i class="fa fa-globe"></i>List Validasi SLF Kepala Dinas</div>
...
<div class="modal fade" id="dialog-popup" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  	<div class="modal-dialog modal-lg">
 		<form action="<?php echo site_url('Penerbitan/ValidasiFormSLF'); ?>" class="form-horizontal" role="form" method="post">
...
function GetCetakSLF(id)
	{
		var url = "<?php echo base_url() . index_page() ?>Penerbitan/CetakSertifikatLaikFungsiBaru/"+id;
		swin = window.open(url,'win','scrollbars,width=1000,height=600,top=80,left=140,status=yes,toolbar=no,menubar=yes,location=no');
		swin.focus();
	}
function GetCetak(id)
{
	$("#MyModalBody").html('<iframe src="<?php echo base_url();?>Penerbitan/CetakSertifikatLaikFungsiBaru/'+id+'" frameborder="no" width="860" height="540"></iframe>');
	$('[name="id"]').val(id) ; 
}
</script>