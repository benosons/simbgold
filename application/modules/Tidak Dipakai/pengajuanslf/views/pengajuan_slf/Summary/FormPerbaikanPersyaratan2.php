...
function Getut(id,f){
	...
	swin.focus();
		...
		<input type="hidden" class="form-control" value="<?php echo set_value('id_slf', (isset($id) ? $id : ''))?>" name="id" placeholder="id" autocomplete="off">
							...
							if(!empty($MasterAdministrasi)){
								...
								foreach ($MasterAdministrasi->result() as $key) {
									...
									<tr class="<?=$clss?>" >
										...
										<?php echo form_open_multipart('pengajuanSLF/persyaratan_submitadmrev/'.$id.'/'.$key->id_syarat.'/1/'.$id_administrasi,array('name'=>'frmup'.$no, 'id'=>'frmup'.$no)); ?>
											...
											<? }else{?>
												<a class="btn default btn-xs blue-stripe" onClick="javascript:Getot('<?php echo $id;?>','<?php echo $dir_file; ?>')" src="<?php echo base_url()?>assets/images/pdf.gif" title='File Pdf' class='link' > 
												Lihat</a>| 
												<a href="<?php echo site_url('pengajuanSLF/removeDataAdministrasirev/'.$id_administrasi.'/adm/'.$id);?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin Hapus Data ini?')" title="Hapus Data"><span class="glyphicon glyphicon-trash"></span></a>
											<? }?>
										<?php echo form_close(); ?>
										</td>
						...
						if(!empty($MasterTeknis))
							{
													...
													foreach ($MasterTeknis->result() as $key) {
															...
															if(!empty($DataTeknis)){
																if(!empty($DataTeknis)){
																		foreach ($DataTeknis->result() as $keyChild) {
																			$file = $keyChild->dir_file;
																			$id_persyaratan_detail = $keyChild->id_dokumen_permohonan;
																			$status_verifikasi = $keyChild->verifikasi;
																			//$id_data_teknis = $keyChild->id_persyaratan_detail;
																			$id_data_teknis = $keyChild->id;
																			if($key->id_syarat == $id_persyaratan_detail)
																			{
														...
														<tr class="<?=$clss?>" >
															...
															<?php if (trim($status_progress) == 2){?>
															<td align="center" width="10%">
																<?php echo form_open_multipart('pengajuanSLF/persyaratan_submitteknisrev/'.$id.'/'.$key->id_syarat.'/2/'.$id_administrasi,array('name'=>'frmup'.$no, 'id'=>'frmup'.$no)); ?>
																	<? if($dir_file == '' or $dir_file == null){?>
																	...
																	<?}else{?>
																		...
																		| 
																		<a href="<?php echo site_url('pengajuanSLF/removeDataTeknisrev/'.$id_teknis.'/2/'.$id);?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin Hapus Data ini?')" title="Hapus Data"><span class="glyphicon glyphicon-trash"></span></a>
...
<?php if (trim($status_progress) == 2){?>
<button class="col-md-12 btn blue" onClick="window.location.href = '<?php echo base_url();?>PengajuanSLF/simpanperbaikan/<?php echo $id;?>';return false;">Simpan Perbaikan</button>
<hr>
<?}else{?>
