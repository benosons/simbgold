						...
						<div class="col-md-12">
							<div class="portlet box blue">
								<div class="portlet-title">
									<div class="caption">
										<i class="fa fa-gift"></i>Data Pengajuan SLF
									</div>
								</div>
								<? if($DataSummary->id_jenis_permohonan == '') {?>
								<?php
								$nama_per = "";
								$id_status_slf 	= $DataSummary->id_fungsi_slf; //Jenis Permohonan SLF (Pertama/Perpanjangan) Ada Datanya
								$id_status_imb 	= $DataSummary->id_kepemilikan_imb; // Jenis Status IMB (Sudah Memiliki / Belum Memiliki) Ada Datanya
								$lama_kontruksi = $DataSummary->lama_kontruksi; // Lama Kontruksi
								$id_fungsi_bg  	= $DataSummary->id_fungsi_bg_slf; // Jenis Bangunan (Fungsi Hunian / Non Hunian) (Ada Datanya)
								$jmlantai 		= $DataSummary->lantai_slf; // Jumlah Lantai Bangunan (Ada Datanya)
								$Pengawas		= $DataSummary->pengawas_kontruksi; // Pengawas Selama Kontruksi Bangunan (Ada Datanya)
								$PengkajiTeknis	= $DataSummary->id_pemeriksaan_kontruksi; // Pengkaji Teknis  (Ada Datanya)
								$JumlahPengawas = $DataSummary->tahapan_kontruksi; // Apakah Jumlah Pengawas/MK Lebih Dari 1
								
								
								if($DataSummary->lantai_slf == '2'){
									$jmlantai ='1';
								}
								
								if($DataSummary->id_fungsi_bg_slf == '2'){
									$jmlantai ='1';
									$id_fungsi_bg ='1';
								}
								/*if($DataSummary->id_kepemilikan_imb == 2 ){
									if($DataSummary->id_kepemilikan_imb != '' ){
										$jmlantai ='1';
										$PengkajiTeknis ='3';
									}else{
										$jmlantai = $DataSummary->lantai_slf;
										$PengkajiTeknis = $DataSummary->id_pemeriksaan_kontruksi;
									}
								}else if($DataSummary->id_kepemilikan_imb == 2 ){
									if ($DataSummary->id_fungsi_bg_slf == 2){
										$id_fungsi_bg ='1';
									}
								}*/
								...
								$SQLcari = " AND a.id_status_slf = '$id_status_slf' AND a.id_status_imb = '$id_status_imb' AND a.id_fungsi_bg = '$id_fungsi_bg' 
											 AND a.id_pengkaji_teknis = '$PengkajiTeknis' AND a.jmlantai = '$jmlantai' AND a.lama_kontruksi ='$lama_kontruksi'";
									...
									if(isset($query['nama_permohonan']) != ''){
										...
										$this->mpengajuan->update_permohonan($dataIn,$id_permohonan_slf);
										}else{
										$nama_per = "<b><span class='hightlight'>Permohonan Anda Tidak Terdaftar, Silahkan Lengkapi Datanya</span></b>";
										}
								?>
									
								<?}elseif($DataSummary->id_jenis_permohonan <= '51') {?>
								
									<?php
									$nama_per = $DataSummary->id_jenis_permohonan;
									?>
								...
								<div class="portlet box blue">
									...
									<div class="portlet box blue-hoki">
										...
										<a href="#tanahnyaslf" role="button" class="btn red" data-toggle="modal">Tambah Data</a>
										...
										<?php
											...
											?>
													<tr>
														...
														<a href="<?php echo site_url('PengajuanSLF/removeDataTanah/'.$key->id_tanah.'/'.$key->id_permohonan_slf);?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin Hapus Data ini?')" title="Hapus Data"><span class="glyphicon glyphicon-trash"></span></a>
														</td>
													</tr>
											<?php			
												}
									...
									<span class="input-group-btn">
										<button class="btn green" onClick="window.location.href = '<?php echo base_url();?>PengajuanSLF/PermohonanSLFForm/<?php echo $this->uri->segment(3);?>';return false;">Kembali</button>
									</span>
									
									<span class="input-group-btn">
										<button class="btn green" onClick="window.location.href = '<?php echo base_url();?>pengajuanSLF/FormAdministrasiSLF/<?php echo $this->uri->segment(3);?>';return false;">Selanjutnya</button>
									</span>
									</center>
									<br>
							</div>
...
<div id="tanahnyaslf" class="modal fade" role="dialog" aria-hidden="true" data-width="60%" data-backdrop="static" data-keyboard="false">
