			...
			<div class="portlet box blue">
							...
							<?php if (!empty($DataMPE)) {
								...
								foreach ($DataMPE->result() as $key) {
									...
									<tr class="<?= $clss ?>">
										...
										<td align="center">
											<?php echo form_open_multipart('KonsultasiOSS/SaveDokumen/' . $id . '/' . $key->id_detail . '/4/' . $id_teknis, array('name' => 'frmup' . $no, 'id' => 'frmup' . $no)); ?>
											...
											<?php } else {
												...
												<a href="<?php echo site_url('Docreader/PDFRead/' . $dir1); ?>" class="btn default btn-xs blue-stripe" data-toggle="modal" data-target="#modal-edit">Lihat</a>
													|
													<a href="<?php echo site_url('KonsultasiOSS/DeleteMEP/' . $id_teknis . '/tek/' . $id . '/' . $dir_file); ?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin Hapus Data ini?')" title="Hapus Data"><span class="glyphicon glyphicon-trash"></span></a>
												</center>
											<?php } ?>
											<?php echo form_close(); ?>
										</td>
				...
				<center>
					<span class="input-group-btn">
						<button class="btn green" onClick="window.location.href = '<?php echo base_url(); ?>KonsultasiOSS/FormDataTeknis/<?php echo $this->secure->encrypt_url($id); ?>';return false;">Kembali</button>
					</span>
					<?php if ($DataBangunan->id_jenis_permohonan == 9 || $DataBangunan->id_jenis_permohonan == 8){ ?>
							<span class="input-group-btn">
							<button class="btn green" onClick="window.location.href = '<?php echo base_url(); ?>KonsultasiOSS/FormDataHijau/<?php echo $this->secure->encrypt_url($id); ?>';return false;">Selanjutnya</button>
						</span>
					<?php } else { ?>
						<span class="input-group-btn">
