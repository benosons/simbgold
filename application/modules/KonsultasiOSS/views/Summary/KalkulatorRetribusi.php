                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-8">
                        <div class="radio-list">
                          ...
                          <label class="radio-inline">
                            <input type="radio" name="permanensi" class="radio-permanensi" value="2" <?= $id_permanensi == '2' ? 'checked' : '' ?> /> Non Permanen </label>
                        </div>
                      </div>
                    </div>
                  </div>
  ...
  $(document).ready(function() {
    ...
    $(document).on("keyup", ".mt-repeater input[type=number]", function() {
      ...
      $.each(allInputs, function(e) {
        let r = $(this).val();
        res = $(this).val() === '' ? 0 : parseFloat($(this).val());
        total *= res;
      });
      $(".jumlah-retribusi", $parent).val(total.toFixed(3));
      sum();
    });
    ...
    let h = function(num, per) {
      let f = parseFloat($('.parameter-fungsi').text()),
        ...
        j = f * (g + num + (k * h)) * i,
        m = j.toFixed(3),
        l = $('.indeks-integrasi').html(`${f} x (${g} + ${num} + (${k} x ${h})) x ${i} = ${m}`);
      $('.indeks-integrasi').val(m);
