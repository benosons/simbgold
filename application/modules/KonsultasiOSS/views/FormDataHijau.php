	...
	<div class="col-md-12">
		<div class="portlet box blue">
			...
			<div class="portlet box blue">
					...
					<table id="data_administrasi" class="table table-bordered table-striped table-hover">
						<thead>
							<tr>
								...
								<?php } else { ?>
									<th width="45%">Data Teknis Bangunan Gedung Hijau</th>
								<?php } ?>
							...
							<?php if (!empty($DataMPE)) {
								...
								foreach ($DataMPE->result() as $key) {
									...
									<tr class="<?= $clss ?>">
										...
										<td align="center">
											<?php echo form_open_multipart('Konsultasi/SaveDokumen/' . $id . '/' . $key->id_detail . '/6/' . $id_teknis, array('name' => 'frmup' . $no, 'id' => 'frmup' . $no)); ?>
				...
				<!-- End Data Teknis MEP-->
				<center>
					<?php if($DataBangunan->id_jenis_permohonan =='8' || $DataBangunan->id_jenis_permohonan =='9'){ ?>
						<span class="input-group-btn">
							<button class="btn green" onClick="window.location.href = '<?php echo base_url(); ?>Konsultasi/FormDataMEP/<?php echo $this->secure->encrypt_url($id); ?>';return false;">Kembali</button>
						</span>
					<?php } else { ?>
						<span class="input-group-btn">
