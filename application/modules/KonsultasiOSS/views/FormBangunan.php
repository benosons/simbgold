<div class="portlet light bordered margin-top-20">
	...
	<div class="portlet-body form">
		...
		<form action="<?php echo site_url('KonsultasiOSS/saveBangunan'); ?>" class="form-horizontal" role="form" method="post" id="FormBangunan">
			...
			<div class="form-actions">
				<center>
					...
					<button class="btn red" onClick="window.location.href = '<?php echo base_url(); ?>KonsultasiOSS/FormPendaftaran/<?php echo $this->secure->encrypt_url($id); ?>';return false;">Kembali</button>
				</center>
			</div>
		</form>
	</div>
...
<script>
	...
	$('#nama_kelurahan').change(function() {
		...
		$.ajax({
			type: 'POST',
			url: '<?php echo base_url(); ?>KonsultasiOSS/getDataOtorita/' + v,
			data: $('form.form-horizontal').serialize(),
			success: function(response) {
				let data = JSON.parse(response);
				let csrf_token = data.csrf;
	...
	function set_jns_bg(v) {
		if (v == 6) {
			...
			// jQuery.post(base_url + 'KonsultasiOSS/getDataJnsBg/' + v, function(data) {
			// 	var jenis_bg = '<option value="">-- Pilih --</option>';
