...
<div class="portlet light margin-top-20" style="min-height:550px;">
	<div class="portlet-title tabbable-line">
		...
		<div class="actions">
			<a href="<?php echo site_url('Pupr/Data_Tpa/'); ?>" class="btn btn-success"><i class="fa fa-save"></i> Selesai </a>
		</div>
	</div>
...
<script>
	...
	$(document).ready(function() {
		...
		table = $("#myTable").DataTable({
			...
			ajax: {
				url: `${site_url}Pupr/Tampil_Pilih_Tpa`,
				type: "post",
				data: function(data) {
		...
		function tampil_data_pilihTPA() {
			$.ajax({
				type: 'ajax',
				url: '<?php echo base_url() ?>Pupr/Tampil_Pilih_Tpa',
				async: false,
				dataType: 'json',
				//cache	: false,
		...
		$('#btn_pilih').on('click', function() {
			...
			$.ajax({
				type: "get",
				url: "<?php echo base_url('Pupr/Pilih_Simpan_Tpa') ?>",
				dataType: "JSON",
				data: {
					kode: kode
				},
		...
		$('#provinsi').change(function() {
			...
			jQuery.get(base_url + 'Pupr/getDataKabKota/' + v, function(data) {
