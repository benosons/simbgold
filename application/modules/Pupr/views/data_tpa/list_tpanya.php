...
<div class="portlet light margin-top-20" style="min-height:550px;">
	<div class="portlet-title tabbable-line">
		<div class="caption caption-md">
			...
			<span class="caption-subject text-primary bold uppercase"><i class=""></i>List Data TPA <?php echo $nama_kabkota; ?></span>
		</div>
		<div class="actions">
			<?php if($nama_kabkota =='KAB. UJI COBA'){ ?>
				<a href="#" class="btn btn-primary"><i class="fa fa-plus"></i> Akun Uji Coba Tidak Bisa Memilih Dari Database TPA</a>
				<?php } else { ?>
				<a href="<?php echo site_url('Pupr/Pilih_Tpa/'); ?>" class="btn btn-primary"><i class="fa fa-plus"></i> Pilih TPA dari Database TPA</a>
			<?php } ?>
		</div>
	</div>
...
<script>
	...
	$(document).ready(function() {
		...
		function tampil_data_TPA() {
			$.ajax({
				type: 'get',
				url: '<?php echo base_url() ?>Pupr/Tampilin_Tpa',
				async: false,
				dataType: 'json',
				//cache	: false,
		...
		$('#btn_hapus').on('click', function() {
			...
			$.ajax({
				type: "get",
				url: "<?php echo base_url('Pupr/Pilih_Hapus_Tpa') ?>",
				dataType: "JSON",
				data: {
					kode: kode
				},
