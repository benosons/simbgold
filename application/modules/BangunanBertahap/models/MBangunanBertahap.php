...
class MBangunanBertahap extends CI_Model
{

    protected $selectDetail = "a.id as id_pemilik,c.id_jenis_bg,c.lama_proses,b.id_prov_bgn,b.luas_basement,b.lapis_basement,b.id_izin,
    ...
    public function getListPenjadwlanDok($SQLcari = '')
    {
        $Dinas = $this->session->userdata('loc_id_kabkot');
        $this->db->select('a.id,a.nm_pemilik,b.status,b.no_konsultasi,b.almt_bgn,b.tahap_pbg,c.nm_konsultasi,g.fungsi_bg,b.tgl_pernyataan');
        $this->db->from('tmdatapemilik a');
    ...
    {
        $fieldDinas = dinas() == '31' ? 'c.id_prov_bgn' : 'c.id_kabkot_bgn';
        $this->datatables->select('a.id,b.nm_konsultasi,c.id_fungsi_bg,c.no_konsultasi,a.nm_pemilik,c.almt_bgn,c.status,c.id_kabkot_bgn')
            ->from('tmdatapemilik as a,tr_konsultasi as b,tmdatabangunan as c')
            ->where('a.id = c.id')
            ->where('b.id = c.id_jenis_permohonan')
            ->where('c.status >= 4')
            ->where('c.id_fungsi_bg', 1)
            ->where($fieldDinas, dinas())
            ->generate();
    }

    public function cekDataBangunan($id)
    {
        return $this->db->select($this->selectDetail)
            ->from('tmdatapemilik a')
            ->where('b.id', $id)
            ->join('tmdatabangunan b', 'a.id = b.id', 'LEFT')
            ->join('tr_konsultasi c', 'b.id_jenis_permohonan = c.id', 'LEFT')
            ->join('tr_kecamatan e', 'e.id_kecamatan = a.id_kecamatan', 'LEFT')
            ->join('tr_kabkot h', 'h.id_kabkot = a.id_kabkota', 'LEFT')
            ->join('tr_provinsi i', 'i.id_provinsi = a.id_provinsi', 'LEFT')
            ->join('tr_fungsi_bg f', 'f.id_fungsi_bg = b.id_fungsi_bg', 'left')
            ->join('tr_kecamatan g', 'g.id_kecamatan = b.id_kec_bgn', 'LEFT')
            ->join('tr_kabkot j', 'j.id_kabkot = b.id_kabkot_bgn', 'LEFT')
            ->join('tr_provinsi k', 'k.id_provinsi = b.id_prov_bgn', 'LEFT')
            ->join('tr_klasifikasi_bg l', 'l.id_klasifikasi_bg = c.klasifikasi_bg', 'LEFT')
            ->join('tr_prasarana m', 'm.idp = b.id_prasarana_bg', 'LEFT')
            ->order_by('b.status', 'asc')->get();
    }
    ...
    public function  getListKonsultasi()
    {
        ...
        $this->db->where("c.id_jenis_permohonan = 8");
    ...
    {
        $this->db->select('a.id as id_pemilik,b.tahap_pbg,a.no_ktp,a.nm_pemilik, a.email,a.no_hp, a.id_kabkota, a.alamat,a.id_kelurahan,
        a.jns_pemilik, a.id_kecamatan,a.id_provinsi, a.no_ktp,a.glr_depan,
        a.glr_belakang, a.jenis_id,
        b.status,b.id as id_bgn,
        b.luas_basement,b.lapis_basement,
        b.id_fungsi_bg,
        b.tipeA,b.luasA,b.tinggiA,b.lantaiA,b.jumlahA,
        b.id_jenis_permohonan,
        b.no_konsultasi,
        b.almt_bgn,b.id_kec_bgn,b.id_kabkot_bgn, b.id_prov_bgn,
        b.luas_bgn,b.tinggi_bgn,b.jml_lantai,
        b.luas_bgp,b.tinggi_bgp,
        h.nama_kabkota,
        ...
        b.id_prototype,
        b.id_prasarana_bg,
        b.id_izin,b.no_imb, b.imb,b.no_slf,b.slf,b.permohonan_slf,b.jual,b.id_doc_tek
        ');
        $this->db->from('tmdatapemilik a');
    ...
    public function getSyaratList($per = NULL, $kls = NULL, $tahap = NULL)
    {
        $this->db->distinct();
        $this->db->select('a.id_persyaratan,a.id,a.id as id_jenis_permohonan,a.id_jenis_persyaratan,a.id_detail_jenis_persyaratan,b.id_detail,b.id_syarat,c.nm_dokumen,c.keterangan,e.dir_file,c.id_tahap');
        ...
        if ($per != null || trim($tahap) != '')  $this->db->where('c.id_tahap', $tahap);
    ...
    public function getSyaratListId($per = NULL, $kls = NULL,$tahap = NULL)
    {
        $this->db->distinct();

        $this->db->select('a.id,a.id_detail_jenis_persyaratan,b.id_syarat,b.id_detail,c.nm_dokumen,c.id_tahap');
        $this->db->from('tr_konsultasi_syarat a');
        $this->db->join('tr_pbg_syarat_detail b', 'b.id_persyaratan = a.id_persyaratan', 'left');
        $this->db->join('tr_dokumen_syarat c', 'c.id = b.id_syarat', 'left');
        $this->db->join('tmdatabangunan d', 'a.id = d.id_jenis_permohonan', 'left');
    ...
    public function insertHasilKonsultasi($data)
    {
        return $this->db->insert('tmdatavaltek', $data);
    }
