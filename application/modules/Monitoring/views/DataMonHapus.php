<div class="portlet box blue">
	<div class="portlet-title">
		<div class="caption"><i class="fa fa-globe"></i>List Data Permohonan PBG</div>
		<div class="tools"><a href="javascript:;" class="reload"></a></div>
	</div>
	<div class="portlet-body">
		<div class="form-actions">
			<?php echo form_open('Monitoring/MonHapus', array('name' => 'frmListVerifikasi', 'id' => 'frmListVerifikasi')); ?>
			<div class="row">
				<div class="col-md-12">
				...
				<?php if ($Verifikasi->num_rows() > 0) {
					...
					foreach ($Verifikasi->result() as $Konsultasi) { ?>
						...
						<tr class="<?= $clss ?>">
							...
							<td align="center"><?php echo $Konsultasi->status_dinas; ?></td>
							<td align="center">
                                <a href="<?php echo site_url('Monitoring/removeDataPengajuan/' . $Konsultasi->id); ?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin Hapus Data ini?')" title="Hapus Data"><span class="glyphicon glyphicon-trash"></span></a>
							</td>
						</tr>
				<?php }
				} ?>
			...
			$.ajax({
				...
				success: function(response) {
					if (response.res == true) {
						...
						$(".alamat-bangunan").html(`${response.almt_bgn}, Kec. ${response.nama_kec_bg}, <br>${response.nama_kabkota_bg}, ${response.nama_provinsi_bg}`);
						...
						if (response.tanah != 0) {
							response.tanah.forEach(obj => {
								...
								let berkas = obj.dir_file == false ? 'Tidak ada dokumen' : `<a href="javascript:void(0);" class="btn btn-success btn-sm" title="Lihat Berkas" onclick="javascript:popWin('${site_url}file/Konsultasi/${response.id_pemilik}/data_tanah/${obj.dir_file}')"><span class="glyphicon glyphicon-file"></span></a>`;
								let pemanfaatan = obj.dir_file_phat == false ? 'Tidak ada dokumen' : `<a href="javascript:void(0);" class="btn btn-success btn-sm" title="Lihat Berkas" onclick="javascript:popWin('${site_url}file/Konsultasi/${response.id_pemilik}/data_tanah/${obj.dir_file_phat}')"><span class="glyphicon glyphicon-file"></span></a>`;
						...
						if (response.id_izin != 2) {
							...
							response.syarat_tanah.forEach(obj => {
								...
								let berkas = obj.dir_file == false ? 'Tidak ada dokumen' : `<a href="javascript:void(0);" class="btn default btn-xs blue-stripe" title="Lihat Berkas" onclick="javascript:popWin('${site_url}file/Konsultasi/${response.id_pemilik}/Dokumen/${obj.dir_file}')"><span class="glyphicon glyphicon-file"></span>Lihat</a>`;
