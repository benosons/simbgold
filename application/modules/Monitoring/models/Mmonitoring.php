...
class Mmonitoring extends CI_Model{
	...
	public function  getListVerifikator($SQLcari = '')
	{
		...
		$this->db->select('a.id,a.nm_pemilik,b.status,b.no_konsultasi,b.almt_bgn,b.nm_bgn,c.nm_konsultasi,g.fungsi_bg,b.tgl_pernyataan,h.status_dinas');
	...
	public function  getListVerifikatorBalai($SQLcari = '')
	{
		$Dinas = $this->session->userdata('loc_id_kabkot');
		...
		if($Dinas =='31'){
			...
			$this->db->where('b.id_prov_bgn', $Dinas);
		}
		$this->db->join('tmdatabangunan b', 'a.id = b.id', 'LEFT');
		$this->db->join('tr_konsultasi c', 'b.id_jenis_permohonan = c.id', 'LEFT');
		$this->db->join('tr_kecamatan d', 'a.id_kecamatan = d.id_kecamatan', 'LEFT');
