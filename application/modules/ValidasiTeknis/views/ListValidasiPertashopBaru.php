...
<div class="portlet box blue">
    <div class="portlet-title">
        <div class="caption"><i class="fa fa-globe"></i>List Validasi Kepala Dinas Teknis untuk Bangunan Pertashop Baru</div>
                        ...
                        <tr class="<?= $clss ?>">
                            ...
                            <?php if ($Kadis->status == 10) { ?>
                                <td align="center">
                                    ...
                                    <div class="data-action"></div><br>
                                    <?php if($Kadis->imb == 1){ ?>
                                        <a href="<?php echo site_url('ValidasiTeknis/RollbackKadisPrasaranaEksis/' . $Kadis->id); ?>" class="btn btn-warning btn-sm" onclick="return confirm('Yakin Mengembalikan Permohonan ini ke Proses Perhitungan Retribusi?')"title="Dikembalikan ke Proses Perhitungan Retribusi"><span class="glyphicon glyphicon-edit"></span>
                                    <?php }else{ ?>
                                        <a href="<?php echo site_url('ValidasiTeknis/RollbackKadisPrasaranaEksis/' . $Kadis->id); ?>" class="btn btn-warning btn-sm" onclick="return confirm('Yakin Mengembalikan Permohonan ini ke Proses Perhitungan Retribusi?')"title="Dikembalikan ke Proses Perhitungan Retribusi"><span class="glyphicon glyphicon-edit"></span>
                                    <?php } ?>
                                </td>
...
<div id="dialog-popup" class="modal fade bs-modal-sm" data-width="55%" tabindex="-1" aria-hidden="true" role="dialog" data-backdrop="static" data-keyboard="false">
    <div class="modal-content">
        <form action="<?php echo site_url('ValidasiTeknis/FormVerifikasiPertaShopBaru'); ?>" class="form-horizontal" role="form" method="post">
...
<script>
    ...
    function GetVerifikasi(id) {
        $("#MyModalBody").html('<iframe src="<?php echo base_url(); ?>Dokumen/CetakVerifikasiBangunan/' + id + '" frameborder="no" width="860" height="540"></iframe>');
        $('[name="id"]').val(id);
    }
</script>
<script>
                ...
                success: function(response) {
                    setTimeout(function() {
                        if (response.res == true) {
                            ...
                            $(".alamat-pemilik").html(`${response.alamat}, Kel/Desa. ${response.nama_kelurahan}, Kec. ${response.nama_kecamatan},${response.nama_kabkota}, ${response.nama_prov_pemilik}`);
                            $(".alamat-bangunan").html(`${response.almt_bgn}, Kel/Desa. ${response.nm_kelurahan}, Kec. ${response.nama_kec_bg},${response.nama_kabkota_bg}, ${response.nama_provinsi_bg}`);
                            $(".fungsi-bangunan-gedung").html(`${response.fungsi_bg}`);
                            $(".luas-tinggi-lantai").html(`${response.luas_bgp} m<sup>2</sup>, dengan tinggi ${response.tinggi_bgp} meter, dan berjumlah ${response.jml_lantai} lantai.`);
