<form action="<?php echo site_url('Sekretariat/simpan_personil_tpa'); ?>" class="form-horizontal" role="form" method="post" id="form_edit_skteknis">
            ...
            if (isset($query_syarat_adm) != 0) :
                ...
                foreach ($query_syarat_adm->result() as $row) :
                    if ($element == 'checkbox') {
                        ...
                        $input    = '<input type="checkbox" id="checkbox' . $row->id_tpanya . '" class="form-control md-check" ' . $setVal . ' name="dok_persyaratan[]" value="' . $row->id_tpanya . '" ' . $set . '>';
                    }
                    $var .= '<div class="md-checkbox col-md-4">';
                    $var .= $input;
                    $var .= '<label for="checkbox' . $row->id_tpanya . '">';
