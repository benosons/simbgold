<div class="portlet box blue">
    ...
    <div class="portlet-body">
        <table class="table table-striped table-bordered table-hover" id="tableInputInspeksi">
            ...
            <tbody>
                <?php
                $no = 1;
                foreach ($Penugasan->result() as $pbg) {
                    $bgcolor = "";
                    $ustat = "";
                    if ($pbg->status <= 18) {
                        ...
                        $bgcolor = "success";
                        $ustat = "Proses Inspeksi Selesai";
                    } ?>
                    <tr class="<?= $bgcolor ?>">
                        <td align="center"><?php echo $no++; ?></td>
                        <td><?php echo $pbg->nm_konsultasi; ?></td>
                        <td align="center"><?php echo $pbg->no_izin_pbg; ?></td>
                        <td><?php echo $pbg->nm_pemilik; ?></td>
                        <td><?php echo $pbg->almt_bgn; ?></td>
                        <td>
                            <center><a href="javascript:;" class="btn btn-info btn-sm detail-verifikasi" title="Lihat Data" data-id="<?= $pbg->id ?>"><span class="glyphicon glyphicon-user"></span></a></center>
                        </td>
                        <td align="center">
                            <?php echo $ustat; ?>
                        </td>
                        </td>
                        <?php if ($pbg->status <= 18) { ?>
                            <td align="center">
                                <a href="<?= site_url("Inspeksi/PemeriksaanInspeksi/{$pbg->id}") ?>" class="btn btn-warning btn-sm" title="Lihat Detail Inspeksi"><span class="fa fa-edit"></span></a>
                            </td>
                        <?php } else { ?>
                            <td align="center">
...
<div id="modal-detail" class="modal fade" data-width="54%" tabindex="-1" aria-hidden="true" role="dialog" data-backdrop="static" data-keyboard="false" data-focus-on="input:first">
...
<script>
    var site_url = "<?= site_url() ?>";
	$(document).ready(function() {
		var table = $('#tableInputInspeksi').DataTable({
			"responsive": false,
			"language": {
				"aria": {
					"sortAscending": ": activate to sort column ascending",
